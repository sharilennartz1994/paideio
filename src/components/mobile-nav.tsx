...
type Entry = { href: string; label: string; icon: typeof Home; badge?: number };
...
function primaryTab(role: Role): Entry {
...
function sheetSections(role: Role, unreadCount: number) {
  const naviga: Entry[] = [
    { href: "/academy", label: "Academy", icon: GraduationCap },
    { href: "/circuito", label: "Circuito", icon: Trophy },
  ];
  if (role === "player") {
    naviga.push({ href: "/preferiti", label: "Preferiti", icon: Heart });
  }
  if (role === "coach") {
    naviga.push({ href: "/prenotazioni", label: "Le mie lezioni", icon: CalendarDays });
  }
  if (role) {
    naviga.push({ href: "/notifiche", label: "Notifiche", icon: Bell, badge: unreadCount });
  }

  const scopri: Entry[] = [
    role === "coach"
      ? { href: "/coach-admin", label: "Area coach", icon: LayoutDashboard }
      : { href: "/diventa-coach", label: "Diventa coach", icon: Trophy },
    { href: "/chi-siamo", label: "Il concept", icon: Info },
    { href: "/prossime-release", label: "Prossime release", icon: Sparkles },
  ];

  return { naviga, scopri };
}
...
export function MobileNav({ role, unreadCount }: { role: Role; unreadCount: number }) {
  ...
  const { naviga, scopri } = sheetSections(role, unreadCount);
  const inSheet = [...naviga, ...scopri].some((entry) => isActive(pathname, entry.href));
      ...
      {tabs.map((tab) => {
        ...
        return (
          <Link
            ...
            className={cn(tabClass, active ? "text-game-ball" : "text-game-white/65 hover:text-game-ball")}
          >
      ...
      <Sheet>
        <SheetTrigger
          render={
            <button
              type="button"
              className={cn(tabClass, "relative", inSheet ? "text-game-ball" : "text-game-white/65")}
            />
          }
        >
        ...
        >
          <SheetHeader className="flex-row items-start justify-between gap-4 px-0">
            <div className="flex flex-col gap-0.5">
              ...
              <SheetDescription className="text-sm text-game-white/65">
            ...
            <SheetClose
              render={
                <button
                  type="button"
                  className="flex min-h-11 min-w-11 items-center justify-center border border-game-cyan/40 font-heading text-xs font-bold text-game-white/75 uppercase transition-colors hover:border-game-cyan hover:text-game-white"
                />
              }
            >
              Chiudi
          ...
          <SheetGroup title="Sezioni" entries={naviga} pathname={pathname} />
          <SheetGroup title="Paideio" entries={scopri} pathname={pathname} />
        </SheetContent>
      </Sheet>
    </nav>
  );
